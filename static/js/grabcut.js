...
function makeProbableForegroundFabricCanvas(myCanvasObj, appendToCol) {
  ...
  fabric.Image.fromURL(myCanvasObj.imageURL, function (img) {
    ...
    canvas_height =
      img._originalElement.height * (canvas_width / scale_determine);
    console.log(canvas_height);
    console.log(img._originalElement.height * (canvas_width / scale_determine));
    console.log("yoyo2");
  ...
  fabric.Image.fromURL(myCanvasObj.imageURL, function (img) {
    ...
    let scale_determine =
      original_height > original_width ? original_height : original_width; // в целом лишнее, так как просто растянем на максимум по ширене экрана
...
function makeGrabCutRow(appending_container, imageURL, project_name) {
  ...
  $(title1).append(
    "<div class='title'>1. Выделите фрагмент для работы <br></div>"
  );
  // $(title2).append("<div class='title'>Result of grabcut with rectangle:</div>")
  $(title2).append(
    "<div class='title'>2. Выделите то, что точно не нужно вырезать.<br> 3. Выделите то, что точно нужно вырезать.</div>"
  );
  ...
  var fg_fabric_canvas = makeProbableForegroundFabricCanvas(
    obj.canvas1,
    col1 /*append to col1 */
  );

  let do_grabcut_button = $(
    "<button class='btn btn-primary'>Run grabcut &nbsp <span class= 'glyphicon glyphicon-scissors'></span></button>"
  );
  ...
  $(do_grabcut_button).click(function () {
    ...
    let row_id = id;
    do_grabcut_backend(
      imageURL.replace("../static/images/", ""),
      rect_1,
      col2,
      appending_container,
      obj
  ...
  let bg_drawing_canvas = makeDrawingFabricCanvas(
    obj.refinement_canvas,
    col3,
    "black"
  );
  ...
  let add_back_detail_button = $(
    "<button class='btn btn-primary'>Run grabcut refinement</button>"
  );
  ...
  let clear_canvas_button = $(
    "<button class='btn btn-primary'>Clear Drawings</button>"
  );
  $(col3)
    .append("<span>&nbsp&nbsp&nbsp&nbsp</span>")
    .append(clear_canvas_button);
